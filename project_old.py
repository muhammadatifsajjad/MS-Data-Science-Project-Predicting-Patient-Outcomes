...
#         continue
#     without_zero_list.append(data_new[id])
#
#One-hot
...
# #400维度
# y_result['count']=1
# y_result = y_result.set_index('y',append = True)
#
# y_result = pd.Series(y_result.values.reshape(len(y_result)),index = y_result.index)
# y_result = y_result.unstack()
# y_result.shape
# y_result = y_result.fillna(0)
# for i in range (1,len(disease_list)+1):
#     if i not in list(y_result.columns):
#         y_result[i]=0
# y_result.shape
# y_result = y_result.reindex(sorted(y_result.columns), axis=1)
# for i in range(1,len(disease_list)+1):
#     y_result[i]=y_result[i].astype(np.int64)
# new_transform = without_zero
# new_transform = new_transform.reset_index(drop=True)
# for i in range(1,len(disease_list)+1):
#     new_transform[i]=new_transform[i].astype(np.int64)
# new_transform = pd.concat([new_transform,y_result], axis=1)
# new_transform.shape
# x = new_transform.iloc[:, :-420]
# y = new_transform.iloc[:, -420:]
...
#     if test_1[i] not in new_new:
#         new_new.append(test_1[i])
